...
const { setCors, parseDateParam, readRequestUrl, isAuthorizedCron } = require('./http-utils');
...
async function handleAvailabilityGet(req, res) {
  ...
  try {
    ...
    res.statusCode = 500;
    res.setHeader('Content-Type', 'application/json');
    res.end(JSON.stringify({ error: 'Failed to load availability' }));
  }
}
...
async function handleAvailabilitySyncRequest(req, res) {
  ...
  if (method === 'GET' && process.env.CRON_SECRET && !isAuthorizedCron(req)) {
    setCors(res, origin, 'GET, POST, OPTIONS');
    res.statusCode = 401;
    res.setHeader('Content-Type', 'application/json');
